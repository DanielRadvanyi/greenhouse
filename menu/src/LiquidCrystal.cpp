...
void delayMicroseconds(unsigned int us)
{
	...
	uint32_t clockRate = Chip_Clock_GetSystemClockRate();
	//uint32_t clockRate = SystemCoreClock;
	double compValDouble = static_cast<double>(clockRate) * s;

	// compare value
	uint64_t compVal = static_cast<uint64_t>(compValDouble);

	//uint64_t compVal = static_cast<uint64_t> (Chip_Clock_GetSystemClockRate() * us / 1000000);
	//uint64_t compVal = static_cast<uint64_t> (SystemCoreClock * us / 1000000);
	...
	// wait until RIT interrupt flag is set - poll the flag in a loop
	while (!Chip_RIT_GetIntStatus(LPC_RITIMER)) {
		//uint64_t counter = Chip_RIT_GetCounter(LPC_RITIMER);

		/*
		Board_UARTPutSTR(to_string(counter).c_str());
		Board_UARTPutSTR("\r\n");
		*/
	};
